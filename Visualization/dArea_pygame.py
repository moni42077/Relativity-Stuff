...
while True:
    ...
    for x in np.round(np.linspace(250, 250 + WIDTH, int(WIDTH / dx) + 1), precision):
        for y in np.round(
            np.linspace(250, 250 + HEIGHT, int(HEIGHT / dy) + 1), precision
        ):
    ...
    for x in np.round(np.linspace(20, 20 + WIDTH, int(WIDTH / dx) + 1), precision):
        for y in np.round(
            np.linspace(20, 20 + HEIGHT, int(HEIGHT / dy) + 1), precision
        ):
            # draw dx dy rectangle
            g_xx = metric(x, y)[0][0]
            g_yy = metric(x, y)[1][1]
            ...
            pg.draw.line(
                SCREEN, BLUE, (x + dx_new, y), (x + dx_new + x_offset, y + y_offset)
            )
            pg.draw.line(
                SCREEN,
                BLUE,
                (x + x_offset, y + y_offset),
                (x + dx_new + x_offset, y + y_offset),
            )
